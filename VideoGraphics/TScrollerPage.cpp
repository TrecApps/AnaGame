...
void TScrollerPage::RefreshScroll()
{
	...
	D2D1_RECT_F childLocation = childPage->GetArea();
	if (!vScroll.Get() && ((area.bottom - area.top) < (childLocation.bottom - childLocation.top)))
	{
		// We need to create the Vertical ScrollBar
		vScroll = TrecPointerKey::GetNewTrecPointer<TScrollBar>(TrecPointerKey::GetTrecPointerFromSoft<>(self), ScrollOrient::so_vertical);
	}

	if (!hScroll.Get() && ((area.right - area.left) < (childLocation.right - childLocation.left)))
	{
		// We need to create the horizontal ScrollBar
		hScroll = TrecPointerKey::GetNewTrecPointer<TScrollBar>(TrecPointerKey::GetTrecPointerFromSoft<>(self), ScrollOrient::so_horizontal);
	}
